import { directionFeaturesFromValues } from "./direction-model.ts";
...
const marketUpMidpoint = (row: DirectionSnapshot) => {
...
const featuresForSnapshot = (row: DirectionSnapshot) => directionFeaturesFromValues({
...
export const buildDirectionExamples = (rows: DirectionSnapshot[]) => {
  const byMarket = new Map<string, DirectionSnapshot[]>();
  ...
  const examples: DirectionExample[] = [];
...
export const chronologicalMarketSplit = (examples: DirectionExample[]) => {
